...
export default function BookingForm() {
  ...
  useEffect(() => {
    try {
      const stored = sessionStorage.getItem("onlineTshirtRequest");
      ...
      setOnlineRequestId(request.id);

      setValue("donor_name", request.full_name ?? "");
      ...
      if (requestItems.length > 0) {
        setValue("items", requestItems);

        const quantity = requestItems.reduce(
          (sum: number, item: { quantity: number }) =>
            sum + item.quantity,
          0
        );

        setValue("total_quantity", quantity);
        setValue("total_amount", quantity * PRICE);
      }
    } catch (error) {
  ...
  async function onSubmit(data: BookingFormValues) {
    try {
      ...
      // If this official booking came from an online request,
      // mark that request as approved.
      if (onlineRequestId) {
        const approvalRes = await fetch(
          `/api/tshirt-requests/${onlineRequestId}`,
          {
            method: "PATCH",
            headers: {
              "Content-Type": "application/json",
            ...
            body: JSON.stringify({
              status: "approved",
              official_booking_id: result.booking.id,
            }),
          }
        );
        ...
        if (!approvalRes.ok) {
          console.error(
            "Booking created but online request approval failed:",
            approvalResult
          );

          toast.warning(
            "Booking created, but online request could not be marked approved."
          );
        } else {
          sessionStorage.removeItem("onlineTshirtRequest");
        }
      }

      toast.success(
        `Booking ${result.booking.booking_id} Created Successfully`
      );
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-yellow-700">
                This information was submitted from the public
                T-Shirt request page. Please verify the customer's
                name, mobile number, sizes and quantities before
                creating the official booking.
              </p>

              <p className="mt-2 text-xs font-semibold text-yellow-700">
                No official JMCR booking exists yet.
              </p>
            </div>
          </div>
        </div>
